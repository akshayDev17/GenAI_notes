...
Run Dumper
==========
"Write the complete run transcript to markdown" mechanism, used by
paper_factored_decomposition.py. Only handles accumulation, filename
derivation, and writing the file - the calling script still renders its own
"full prompt sent" text (e.g. history-list style), since that rendering is
...
def results_md_path(implementation_title, model_name, run_label=None):
    """
    ...
    """
    parts = [_slug(implementation_title), _slug(model_name)]
    if run_label:
...
class RunDumper:
    ...
    def write(self):
        ...
        if not self.enabled:
            return None
        path = results_md_path(self.title, self.model_name, self.run_label)
